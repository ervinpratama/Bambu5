...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-md-5">
                                <input type="text" class="form-control" name="order_id" id="order_id"
                                    value="<?= $order_id ?>" placeholder="ORDER ID">
                            </div>
                            <div class="col-md-5">
                                <input type="text" class="form-control" name="nama" id="nama"
                                    value="<?= $nama ?>" placeholder="Nama Customer">
                            </div>
                            <div class="col-md-2">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if (isset($row['status_r']) && $row['status_r'] == 'Selesai')
                                                <button class="btn btn-primary btn-sm" data-gambar="{{ $row['refund_r'] }}"
                                                    id="bukti_refund"><i class="fa fa-eye"></i> Bukti Refund</button>
                                            @elseif($row['status_b'] == 'Selesai')
                                                <button class="btn btn-primary btn-sm" data-gambar="{{ $row['refund_b'] }}"
                                                    id="bukti_refund"><i class="fa fa-eye"></i> Bukti Refund</button>
                                            @endif
                                            <a href="/transaction/detail/{{ $row['id'] }}"
                                                class="btn btn-success btn-sm">Detail</a>
                                            @if (!isset($row['status_b']) || ($row['status_b'] == 'pending' && !isset($row['status_b'])))
                                                <a href="/transaction/upload_bukti_transfer/{{ $row['id'] }}"
                                                    class="btn btn-sm btn-primary">Upload</a>
                                                <a onclick="if (! confirm('Batalkan Pesanan?')) { return false; }"
                                                    href="/transaction/batalkan_pesanan/{{ $row['id'] }}"
                                                    class="btn btn-sm btn-danger">Batalkan</a>
                                            @elseif(isset($row['status_b']) && $row['status_b'] == 'dikirim')
                                                <a onclick="if (! confirm('Terima Pesanan?')) { return false; }"
                                                    href="/transaction/terima_pesanan/{{ $row['id'] }}"
                                            ...
                                            @elseif(isset($row['status_b']) && $row['status_b'] == 'selesai' && !isset($row['status_r']))
                                                <a href="/transaction/reject/{{ $row['id'] }}"
                                                    class="btn btn-sm btn-danger">Reject</a>
                                            @else
                                            @endif

                                            @if (isset($row['status_r']) && $row['status_r'] == 'Reject diterima')
                                                <a href="/transaction/reject_upload/{{ $row['reject_id'] }}"
                                                    class="btn btn-sm btn-danger">Upload Rekening</a>
                                            @endif
                                            @if (isset($row['status_b']) && $row['status_b'] == 'dibatalkan')
                                                <a href="/transaction/batal_upload/{{ $row['bukti_id'] }}"
                                                    class="btn btn-sm btn-danger">Upload Rekening</a>
                                            @endif
                                        </td>
                                    </tr>
    ...
    <!-- Modal -->
    <div class="modal fade" id="buktirefundmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary closes" data-dismiss="modal"
